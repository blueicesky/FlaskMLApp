...
class otc_model:
    ...
    def __init__(self,model_type=1,model_params={'n_estimators':200,'max_depth':10,'max_features':0.4}):
        ...
        if model_type == 1:
            ...
            self.model =  MLPClassifier(solver='adam', \
                ...
                hidden_layer_sizes=(config.getint('NeuralNetConfig','HIDDEN_1')\
                    , config.getint('NeuralNetConfig','HIDDEN_2'), config.getint('NeuralNetConfig','HIDDEN_3'),\
                     config.getint('NeuralNetConfig','HIDDEN_4')), random_state=config.getint('NeuralNetConfig','RSTATE'),\
                      activation=config.get('NeuralNetConfig','ACTIVATION'), max_iter=config.getint('NeuralNetConfig','MAX_ITER'), learning_rate_init=config.getfloat('NeuralNetConfig','LEARNING_RATE'), \
                      verbose=config.getboolean('NeuralNetConfig','VERBOSE'))
        else:
    ...
    def flag(self, df):
        ...
        for col in self.training_data.columns:
            d[col] =dict(enumerate(self.training_data[col].unique().flatten(), 1))
        
        # starttime = time()
        for index, row in df_new.iterrows():
            for col in self.training_data.columns.values:
                if (row[col]) not in set(d[col].values())and col not in\
                ['TRADE_GROUP_ID', 'NOTIONAL','NOTIONAL_2', 'STRIKE_PRICE', 'DIFF_AGE']:
    ...
    def set_training_frame(self,training_frame, training_data):
    ...
    def get_training_frame(self):
        ...
        return [self.training_frame, self.training_data]
    ...
    def re_train(self,filename):
        ...
        if not old_data_x_frame.empty:
            ...
            new_data_x = new_data_x_frame.values
            self.current_report = self.execute_train_report(new_data_x, new_data_y, test_size=config.getfloat('Misc','TEST_SIZE_2'),random_state=config.getint('Misc','RSTATE_1'))
        end = time.time()
    ...
    def execute_train_report(self, X, y, test_size=config.getfloat('Misc','TEST_SIZE_1'),random_state=config.getint('Misc','RSTATE_1')):
        #Split data
        ...
        classification_report_var = classification_report(y_test,pred,digits=config.getint('Misc','DIGIT_NUM'))
